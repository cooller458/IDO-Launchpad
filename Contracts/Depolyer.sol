...
interface IPresaleProxy{
   ...
   function getActivated(address _sale) external returns (bool);
   function getTierInfo(uint256 _id) external returns (uint256,uint256,uint256);
   function getTierLength() external returns (uint256);
}

interface IDeployer{
    function getDepolyer() external returns (address);
}
contract BSCPresale is Owned  {
   ...
   bool public isSuccess = false;
    ...
    constructor(address[] memory _addresses,uint256[] memory _values,bool[] memory _isSet,string[] memory _details) public {
        ...
        // name 5
        // symbol 6
        PROXY = IDeployer(msg.sender).getDepolyer();
    ...
    receive() external payable{
        ...
        // isPresaleOpen = false;
        bool isActivated = IPresaleProxy(PROXY).getActivated(address(this));
        require(isActivated,"Sale is Not Activated");
  ...
  function claimTokens() public{
    ...
    function refund() external returns (bool){
        require(msg.sender == PROXY , "Unauthorised access !");
        if((IBEP20(tokenAddress).balanceOf(address(this))) > 0)
        IBEP20(tokenAddress).transfer(owner, (IBEP20(tokenAddress).balanceOf(address(this))));
...
contract Deployer is Owned {
    ...
    function setDepolyer(address _publisher) public onlyOwner {
        deployerAddress = _publisher;
    }
    ...
    function getDepolyer() public view returns (address) {
     ...
     function depolyPresale(address[] calldata _addresses,uint256[] calldata _values,bool[] memory _isSet,string[] memory _details) public returns (address) {
          // _token 0
        //_router 1
        //owner 2
        //_min 0 
